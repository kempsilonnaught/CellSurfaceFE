...
void SimulateSurface::cell_mesh(double r1, double r2, double epsilon, double sep, double x, double y, bool first_run){
	...
	GridGenerator::hyper_shell(surface, center, r1, r2, 12);

	surface.reset_all_manifolds();
	surface.set_all_manifold_ids(0);
	surface.set_manifold(0, manifold);
			...
			for(; cell_1 != endc_1; ++cell_1){
				for(unsigned int l_1 = 0; l_1 < GeometryInfo<2>::lines_per_cell; ++l_1){
					...
					if(cell_1 -> line(l_1) -> at_boundary()){
						if(sqrt((std::pow((edge_center_1[0]), 2))+std::pow(edge_center_1[1], 2)) <= (r1 + epsilon)){
	...
	surface.set_manifold(1, manifold);

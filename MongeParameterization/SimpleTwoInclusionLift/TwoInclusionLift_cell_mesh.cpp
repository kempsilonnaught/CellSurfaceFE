...
void SolveLaplacian::cell_mesh(double r1, double r2, double s, double x, double y, int n){
	...
	GridGenerator::hyper_cube_with_cylindrical_hole(inclusion_1, inclusion_rad_1, s/2, true);
	GridTools::shift(Point<2>(s/2, 0), inclusion_1);	

	Triangulation<2> inclusion_2;
	GridGenerator::hyper_cube_with_cylindrical_hole(inclusion_2, inclusion_rad_2, s/2, true);
	GridTools::shift(Point<2>(-s/2, 0), inclusion_2);

		...
		for(unsigned int q = 0; q < n_quadp; ++q){
			for(unsigned int i = 0; i < numdofs; ++i){
				for(unsigned int j = 0; j < numdofs; ++j){
					...
					energy += (kappa*((solution(local_dof_indices[i]))*(solution(local_dof_indices[j]))*(trace(hess_i))*(trace(hess_j)))/2);
					energy += (kappabar*((hess_i[0][0])*(solution(local_dof_indices[i]))*(hess_j[1][1])*(solution(local_dof_indices[j]))));
					energy += (-kappabar*((solution(local_dof_indices[i]))*(solution(local_dof_indices[j]))*(hess_i[0][1])*(hess_j[0][1])));
